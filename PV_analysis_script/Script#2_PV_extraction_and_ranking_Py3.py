...
#Generate the dfPVs dataframe from dfvalid with unique PVs/2 columns
    ...
    dfPVs['proportion'] = (dfPVs['# '] / dfPVs['# '].sum()).apply(lambda x: '%.9f' % x)
...
#Generate a file with just the PVs that are unique
    ...
    dfheadnew.columns=[df.columns[3], df.columns[1],'count','proportion']
...
#replace filename with new name from Zuordnung.txt
    ...
    dfPVsexport.to_csv(my_dir + newname + "_PVs.csv")
    dfallexport.to_csv(str(my_dir)+newname+"_all.csv")
    dfvalidexport.to_csv(str(my_dir)+newname+"_validSeq.csv")
